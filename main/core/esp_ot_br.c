...
// Task to monitor Thread state and auto-start commissioner
static void commissioner_monitor_task(void *pvParameters)
{
    bool commissioner_started = false;

    while (1)
    {
        vTaskDelay(pdMS_TO_TICKS(2000)); // Check every 2 seconds

        if (!commissioner_started)
        {
            esp_openthread_lock_acquire(portMAX_DELAY);
            otInstance *instance = esp_openthread_get_instance();
            otDeviceRole role = otThreadGetDeviceRole(instance);

            // Start commissioner when device becomes leader or router
            if (role == OT_DEVICE_ROLE_LEADER || role == OT_DEVICE_ROLE_ROUTER)
            {
                if (otCommissionerStart(instance, NULL, NULL, NULL) == OT_ERROR_NONE)
                {
                    ESP_LOGI(TAG, "Commissioner started automatically (role: %s)",
                             role == OT_DEVICE_ROLE_LEADER ? "leader" : "router");
                    // Add a default joiner entry
                    otCommissionerAddJoiner(instance, NULL, "THREADNET", 0);
                    ESP_LOGI(TAG, "Default joiner added with credential 'THREADNET'");
                    commissioner_started = true;
                }
            }
            esp_openthread_lock_release();
        }
...
void ot_br_init(void *ctx)
{
    ...
    // Register web server event handlers BEFORE network connection
    ESP_LOGI(TAG, "Registering web server event handlers...");
    ...
    // Start background task to monitor Thread state and auto-start commissioner
    xTaskCreate(commissioner_monitor_task, "ot_commissioner", 4096, NULL, 3, NULL);
#endif // CONFIG_OPENTHREAD_BR_AUTO_START
