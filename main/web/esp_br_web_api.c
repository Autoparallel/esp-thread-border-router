...
cJSON *handle_ot_resource_node_coprocessor_version_request()
{
    // For native mode, return version indicating no separate coprocessor
    // The ESP32-C6 itself is running the OpenThread stack
    esp_openthread_lock_acquire(portMAX_DELAY);
    const char *ot_version = otGetVersionString();
    esp_openthread_lock_release();

    cJSON *response = cJSON_CreateObject();
    cJSON_AddItemToObject(response, "version", cJSON_CreateString(ot_version));
    cJSON_AddItemToObject(response, "mode", cJSON_CreateString("native"));
    cJSON_AddItemToObject(response, "description", cJSON_CreateString("ESP32-C6 native OpenThread implementation"));
